...
import { getBookName } from '../../types';

const fetchTopProducts = async () => {
  try {
    const response = await bookService.getTopProducts();
...
const Header: React.FC = () => {
  ...
  const MobileHeader = () => (
    <div className="md:hidden block bg-white border-b border-gray-200" style={{ height: '56px' }}>
      <div className="flex items-center h-full px-4 gap-3">
        ...
        {/* Search Bar */}
        <div
          className="flex items-center flex-1 h-9 border border-gray-300 rounded-lg px-3"
        ...
        >
          <img src="/header/header_cart.png" alt="Giỏ hàng" className="w-6 h-6" />
          <span
  ...
  const DesktopHeader = () => (
    <>
      {/* Top banner */}
      <div
        className="text-green-700 md:block hidden"
        style={{ backgroundColor: '#effff4', padding: '12px 16px', height: '42px' }}
      >
        <div
          className="container-custom text-center text-sm font-bold"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        >
          <span style={{ fontSize: '12px', color: '#00AB56' }}>
            Freeship đơn từ 45k, giảm nhiều hơn với
          </span>
          <img
      ...
      <div className="container-custom py-4 md:block hidden">
        <div className="flex items-start px-2 gap-3">
          ...
          <Link to="/" className="flex-shrink-0 w-32 flex flex-col items-center">
            <img src="/header/logo.png" alt="Tiki Logo" style={{ width: '96px', height: '40px' }} />
            <span
              className="text-center w-24 h-4 text-sm font-bold mt-2"
              style={{ color: '#003ea1', fontFamily: 'Inter, Arial, sans-serif' }}
            >
              Tốt & Nhanh
            </span>
          </Link>
          ...
          <div className="flex-1 min-w-0 h-16">
            <div className="flex items-center gap-4 w-full h-10">
              ...
              <div className="flex items-center space-x-3 h-12 flex-shrink-0 pl-9">
                ...
                <Link
                  to="/"
                  className="flex items-center space-x-2 font-medium px-3 py-2"
                  style={{ fontSize: '14px' }}
                >
                  <img src="/header/header_home.png" alt="Trang chủ" className="w-6 h-6" />
                ...
                <div className="user-menu-container" style={{ position: 'relative' }}>
                  <button
                    ...
                    style={{ fontSize: '14px' }}
                  >
                    <img src="/header/header_account.png" alt="Account" className="w-6 h-6" />
                ...
                <button
                  onClick={handleCartClick}
                  className="relative flex items-center text-gray-400 hover:text-blue-600 px-3 py-2"
                >
                  <img src="/header/header_cart.png" alt="Giỏ hàng" className="w-6 h-6" />
            ...
            <div className="flex gap-3 mt-2 overflow-x-auto" style={{ fontWeight: '400' }}>
              {topProducts && topProducts.length > 0 ? (
                topProducts.map((product) => (
                  <span
                    key={product.id}
                    className="text-sm font-normal text-gray-400 hover:text-blue-600 cursor-pointer whitespace-nowrap py-0.5"
                    style={{ color: '#808089' }}
                  >
                    {getBookName(product).length > 25
                      ? getBookName(product).substring(0, 25) + '...'
                      : getBookName(product)}
                  </span>
                ))
              ) : (
      ...
      <nav className="border-t border-b border-gray-200 md:block hidden" style={{ height: '47px' }}>
        <div className="container-custom h-full">
          <div className="flex items-center h-full px-6">
            <Link
              to="/commitment"
              className="mr-6 hover:text-blue-400"
              style={{ fontSize: '14px', color: '#003ea1', fontWeight: '600' }}
            >
              Cam kết
            </Link>
            ...
            <div className="flex items-center space-x-4">
              <Link to="/guarantee" className="flex items-center space-x-1 hover:text-blue-400">
                ...
                <span
                  ...
                  style={{ fontSize: '12px', color: '#27272a', fontWeight: '550' }}
                >
                  100% hàng thật
                </span>
              </Link>
  ...
  return (
    <>
      <header
        ...
        style={{ fontFamily: 'Inter, Arial, sans-serif', fontWeight: '400', color: '#808089' }}
      >
